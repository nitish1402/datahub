...
RUN chmod a+x /start_datahub_actions.sh && \
    ...
    apt-get update && \
    apt-get install -y -qq default-jre nano vim && \
    apt-get clean && \
...
# RUN chown -R datahub:datahub ./datahub-ingestion/.cache
...
COPY --chown=datahub:datahub ./docker/dist ./dist

RUN UV_LINK_MODE=copy uv pip install ./dist/acryl_datahub-1!0.0.0.dev0-py3-none-any.whl
